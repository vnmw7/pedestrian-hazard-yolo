...
async fn main() {
    ...
    if !model_path.exists() {
        ...
        yolo::download_model(model_path)
            .await
    ...
    // Hugging Face Docker Spaces expose port 7860. PORT remains configurable
    // so the same image can be used by other container platforms.
